...
class BPlusTree:
    ...
    def _search_in_internal_node(self, page_data, key_bytes_to_find):
        ...
        while left <= right:
            mid = (left + right) // 2
            key, _ = srl.read_data_from_slot(page_data, mid)
        ...
        if best_slot != -1:
            _, value_bytes = srl.read_data_from_slot(page_data, best_slot)
            next_page_id = int.from_bytes(value_bytes, 'little')
    ...
    def _insert_data_into_page(self, page_id, page_data, key_bytes, value_bytes, data_block_size, is_overflow=False):
        ...
        while left <= right:
            ...
            if key == key_bytes:
                raise Exception(f"Chave '{key_bytes.decode('utf-8', 'ignore')}' já existe.")
            elif key < key_bytes:
                left = mid + 1
            else:
                right = mid - 1
    ...
    def _delete_from_leaf(self, page_id, page_data, key_bytes_to_delete):
        ...
        # Como as remoções são pontuais e precisamos ler todos para reconstruir a página, 
        # a varredura linear aqui é apropriada e segura.
        for i in range(num_slots):
